...
export function KisayolYardim() {
  // i18n: cookie'den etkin dili al + çeviri yardımcısı.
  const { ceviri: t } = usePanelDil();
  // Kısayol listesi çeviriyle üretilir. "sonra" ayracı özel token olarak korunur.
  const KISAYOLLAR = [
    { tus: ["⌘", "K"], ad: t("kisayol.komutPaleti") },
    { tus: ["G", "sonra", "H"], ad: t("kisayol.genelBakis") },
    { tus: ["G", "sonra", "T"], ad: t("kisayol.trafik") },
    { tus: ["G", "sonra", "K"], ad: t("kisayol.kurallar") },
    { tus: ["?"], ad: t("kisayol.yardim") },
    { tus: ["ESC"], ad: t("kisayol.kapat") },
  ];
        ...
        <div className="fixed inset-0 z-[160] flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center gap-2 border-b border-line px-5 py-4">
              ...
              <h3 id="kisayol-yardim-baslik" className="text-[15px] font-semibold text-slate-ink">{t("kisayol.baslik")}</h3>
            </div>
              ...
              {KISAYOLLAR.map((k) => (
                <div key={k.ad} className="flex items-center justify-between py-3">
                  ...
                  <span className="flex items-center gap-1">
                    {k.tus.map((tus, i) =>
                      tus === "sonra" ? (
                        <span key={i} className="text-[11px] text-slate-faint">{t("kisayol.sonra")}</span>
                      ) : (
                        <kbd key={i} className="rounded-md border border-line-strong bg-canvas px-2 py-0.5 text-[12px] font-medium text-slate-ink">{tus}</kbd>
                      ),
                    )}
                  </span>
                </div>
            ...
            <div className="border-t border-line px-5 py-2.5 text-center text-[12px] text-slate-faint">
              <kbd className="rounded border border-line-strong px-1.5 py-0.5">ESC</kbd> {t("kisayol.ileKapat")}
            </div>
          </motion.div>
        </div>
      )}
