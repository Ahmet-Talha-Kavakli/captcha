...
import {
  ...
  Phone, MapPin, FileText, Landmark,
} from "lucide-react";
...
function FirmaBilgileri() {
  const satirlar: [typeof Building2, string, React.ReactNode][] = [
    [Building2, "Ticaret Unvanı", FIRMA.unvan],
    [MapPin, "Açık Adres", FIRMA.adres],
    [Landmark, "Vergi Dairesi / No", `${FIRMA.vergiDairesi} — ${FIRMA.vergiNo}`],
    [FileText, "MERSIS No", FIRMA.mersis],
    [FileText, "Ticaret Sicil No", FIRMA.ticaretSicilNo],
    [
      Phone,
      "Telefon",
      <a
        ...
        className="text-veylify-700 hover:underline"
      >
        {FIRMA.telefon}
      </a>,
    ],
    [
      Mail,
      "E-posta",
      <a key="mail" href={`mailto:${FIRMA.eposta}`} className="text-veylify-700 hover:underline">
        {FIRMA.eposta}
      </a>,
    ],
    [
      ShieldCheck,
      "KVKK / Veri Sorumlusu",
      <a key="kvkk" href={`mailto:${FIRMA.kvkkEposta}`} className="text-veylify-700 hover:underline">
        {FIRMA.kvkkEposta}
      </a>,
    ],
    [Clock, "Çalışma Saatleri", FIRMA.calismaSaatleri],
  ];
  return (
    <section className="border-t border-veylify-100 px-5 py-16 lg:px-8">
      <div className="mx-auto max-w-4xl">
        <Reveal className="text-center">
          <Badge variant="indigo">
            <Building2 className="size-3.5" /> Firma Bilgileri
          </Badge>
          ...
          <p className="mx-auto mt-3 max-w-xl text-[15px] leading-relaxed text-slate-600">
            6502 sayılı Tüketicinin Korunması Hakkında Kanun kapsamında hizmet sağlayıcıya ait
            açık kurumsal bilgiler aşağıda yer alır.
          </p>
        </Reveal>

        <Reveal delay={1} className="mt-8 overflow-hidden rounded-3xl border border-veylify-100 bg-white shadow-[0_24px_64px_-40px_rgba(79,70,229,0.28)]">
          <dl className="divide-y divide-veylify-100">
            {satirlar.map(([Ikon, etiket, deger]) => (
              <div key={etiket} className="grid gap-1.5 px-6 py-4 sm:grid-cols-[220px_1fr] sm:gap-6">
                <dt className="flex items-center gap-2.5 text-[13px] font-semibold uppercase tracking-wide text-slate-400">
                  <Ikon className="size-4 shrink-0 text-veylify-500" />
                  {etiket}
                </dt>
                <dd className="text-[15px] font-medium text-veylify-950">{deger}</dd>
              </div>
            ))}
          </dl>
        </Reveal>
