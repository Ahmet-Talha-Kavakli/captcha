...
```bash
npm test              # tüm suite (135 test: e2e + güvenlik + edge + vision)
npm run test:e2e      # uçtan-uca akış + adversarial + webhook teslimat (49)
npm run test:security # kimlik/yetki/izolasyon + token/2FA/CORS/plan + SSRF (58)
npm run test:edge     # bozuk input + prototype-pollution + kota-zorlama (28)
npm run test:vision   # gerçek Tesseract OCR körlük kanıtı (ghost-font %0)
```

Server `127.0.0.1:3033`'te çalışırken çalıştırın. Webhook e2e testi loopback
alıcı kullandığından, SSRF koruması nedeniyle sunucuyu şu env ile başlatın
(yalnızca test/geliştirme — production'da ASLA):

```bash
VEYLIFY_ALLOW_LOCAL_WEBHOOK=1 npm start
```
